...
%        outList.
%
function pout=adiabaticEvolution_Fock_reverse_RUN3(pin,outList)
...
% generate state
psiInit=makeState('Eigen','psi',nn2k,N,Dim,H,[c1,c2,c3,c4],a1'*a1+a2'*a2);
...
t=dt;
% calc special time point
t_1=(tmax-2*relaxT)/2;
t_2=t_1+relaxT;
t_3=t_2+t_1;
while(t<=tmax)
    % Update J, Ec and H
    J = (J1+(J2-J1)*t/t_1)          *(t<t_1)...
       +J2                          *(t_1<=t)*(t<=t_2)...
       +(J2+(J3-J2)*(t-t_2)/t_1)    *(t_2<t)*(t<t_3)...
       +J3                          *(t>=t_3);
    Ec= (Ec1+(Ec2-Ec1)*t/t_1)       *(t<t_1)...
       +Ec2                         *(t_1<=t)*(t<=t_2)...
       +(Ec2+(Ec3-Ec2)*(t-t_2)/t_1) *(t_2<t)*(t<t_3)...
       +Ec3                         *(t>=t_3);
    ...
    if (t-tList(rCount)>=spt || t+dt>tmax)
        ...
        set(gca,'ylim',[0 0.6]);
           % calc energy eigen state composition
           N0=N;
           generateEigenStates;   
        title(['t=',num2str(t,'%7.3f'),...
               ...
               '  dt=',num2str(dt,'%6.1e'),...
               '  c_1=',num2str(abs(V(:,1)'*psi{cur})^2,'%6.3f'),...
               '  c_2=',num2str(abs(V(:,2)'*psi{cur})^2,'%6.3f'),...
               '  c_3=',num2str(abs(V(:,3)'*psi{cur})^2,'%6.3f'),...
               '  c_4=',num2str(abs(V(:,4)'*psi{cur})^2,'%6.3f')]);
               %'  Ec=',num2str(Ec,'%6.3f'),...
...
%% Set return values
% if outList is not provided, use default outList
